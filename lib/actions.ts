...
export const isStoryDuplicated = async (story: StoryZod): Promise<boolean> => {
  ...
  results.forEach((result, index) => {
    if (result) {
      ...
      const failedFields = fields.reduce<Record<string, unknown>>(
        (acc, field) => {
          // Support nested fields using dot-notation.
          const value = field
            .split(".")
            .reduce(
              (obj: Record<string, any>, key: string) => obj[key],
      ...
      console.log(
        `Criteria "${name}" detected a duplicate with the following fields:`,
        failedFields
      );
    }
  });
...
export const getPaginatedStoriesMetadata = async ({
  page,
  pageSize,
}: PaginationParams): Promise<StoryMetadataForPrompt[]> => {
  try {
    await dbConnect();
...
export const create_story_ai = async (
  key: string,
  metadata: StoryMetadataForPrompt[]
): Promise<unknown> => {
